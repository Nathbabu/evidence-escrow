...
class EvidenceEscrow(gl.Contract):
    ...
    def submit_evidence(self, evidence: str, evidence_url: str = "") -> None:
        ...
        if self.status == "Funded":
            ...
            self.dispute_opened_at = gl.message_raw["datetime"]
    ...
    def resolve_dispute(self) -> dict[str, typing.Any]:
        ...
        other side has had a real chance to answer, and a silent
        counterparty can't lock the funds up forever either.
        """
        sender = gl.message.sender_address
        if sender != self.payer and sender != self.payee:
        ...
        if not both_responded:
            ...
            now = _parse_datetime(gl.message_raw["datetime"])
        ...
        payee_evidence = self.payee_evidence or "(no written evidence submitted)"
...
If a party submitted no evidence and no working link, weigh that
